...
**Variedad de armas (2026-08-24):** `scripts/util/Weapons.gd` es el espejo exacto de `sim/Weapons.ts` del servidor (mismos ids, radios y daños) — tres armas: **Mini Bazooka** (la de siempre), **Racimo** (explosión primaria chica + 4 sub-proyectiles que se dispersan desde el punto de impacto y explotan por separado, cada uno con su propio radio/daño), y **Rebote** (rebota una vez en el terreno antes de explotar). El HUD arma un selector con un botón por arma (`Hud._setup_weapons()`, señal `weapon_selected`) visible mientras la partida está activa, oculto al terminar. Tanto `Main.gd` (local) como `NetworkMain.gd` (online) escuchan esa señal y usan el arma elegida al soltar el arrastre.
