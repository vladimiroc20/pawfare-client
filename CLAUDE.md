...
Efectos implementados: `shoot` (disparo, barrido descendente — con pitch distinto por arma: racimo más agudo, rebotante más grave, perforador el más agudo), `explosion` (ver detalle abajo, con pitch escalado según el radio de explosión del arma), `bounce` (rebote de la granada rebotante), `hit` (impacto contra roca), `eliminated` (KO de un jugador), `victory` (podio/fin de partida), y `ui_click` (botones de menú, HUD y selección de personaje).

**Explosión, técnica de capas (2026-08-24, ajustado tras feedback — el primer intento sonaba a "siseo de ruido", no a un boom):** la versión inicial usaba un solo ruido blanco con un filtro casero de 2 muestras (demasiado débil) y una caída cúbica — sonaba plano. Se reescribió con la técnica estándar de diseño de sonido para explosiones (grave separado del ruido, golpe de sub-bajo, chasquido inicial), confirmada por investigación web: tres capas sumadas en `_synth_explosion()`: 1) **chasquido** — ruido blanco sin filtrar, ~20ms, el "crac" seco del instante del impacto; 2) **golpe** ("thump") — seno que cae de ~110Hz a ~40Hz en los primeros 200ms con caída rápida (`exp(-t·9)`), el puñetazo grave que se *siente*; 3) **cuerpo** — ruido pasado por un filtro pasa-bajos de un polo real (`lp_state += alpha·(white - lp_state)`, corte ~300Hz) con caída exponencial larga (`exp(-t·3.5)`, duración total 0.9s) para el retumbo. La caída exponencial (no polinomial) y el filtro de un polo (no el promedio de 2 muestras anterior) son la diferencia clave frente al primer intento. Confirmado por el usuario ("mejor!") tras escucharlo en vivo. Commit `c620f59`.
